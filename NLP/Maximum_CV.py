...
#Cross-validation for method 1 (Maximum Normalised Frequency)
...
c = 100 #c-fold cross-validation is conducted on classification accuracy
f = 1000 #number of equal-sized test sets (f >= c)
#e.g. if f = 100, test set = 1% of test data or 0.1% of total data
...
test_set = [(featureExtractor(line), lab) for (line, lab) in labelledTest]
test_setNA = [(featureExtractor(line), lab) for (line, lab) in labelledTestNA]

#creating f equally-sized chunks for cross-validation
test_set_chunk = np.array_split(np.array(test_set), f)
test_set_chunkNA = np.array_split(np.array(test_setNA), f)
...
acc = []
for i in range(c):
    print('Classifier accuracy on test set {}:'.format(i+1))
    acc.append(nltk.classify.accuracy(classifier, test_set_chunk[i]))
    print(acc[i])
...
for i in range(c):
    print('Classifier accuracy on test set {}:'.format(i+1))
    accNA.append(nltk.classify.accuracy(classifierNA, test_set_chunkNA[i]))
    print(accNA[i])
    
print('For classification using data with diacritics preserved...')
print('Average classifier accuracy on test set:')
print(np.mean(acc))
print('Standard deviation of classifier accuracy on test set:')
print(np.std(acc))
print('Coefficient of variance of classifier accuracy on test set:')
print(np.std(acc)*100/np.mean(acc))

print('For classification using data with diacritics removed...')
print('Average classifier accuracy on test set:')
print(np.mean(accNA))
print('Standard deviance of classifier accuracy on test set:')
print(np.std(accNA))
print('Coefficient of variance of classifier accuracy on test set:')
print(np.std(accNA)*100/np.mean(accNA))
